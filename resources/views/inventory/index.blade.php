...
@section('content')
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
        <div>
            <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Inventory</h1>
            <p class="text-sm md:text-base text-gray-500 dark:text-gray-400 font-medium">Track inventory levels and reorder points</p>
        </div>
                    ...
                    @forelse ($items as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                            ...
                            <td class="pl-4 pr-3 py-4 whitespace-nowrap">
                                <span class="px-1 py-1 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-bold">
                                    #{{ $item->ItemID }}
                                </span>
                            </td>
                            ...
                            <td class="px-3 py-4">
                                <div class="flex items-center gap-3 min-w-[100px]">
                                    <div class="hidden sm:flex w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex-shrink-0 items-center justify-center text-indigo-600 dark:text-indigo-400">
                            ...
                            <td class="hidden md:table-cell px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <td class="px-3 py-4 whitespace-nowrap">
                                <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                                    <span class="text-sm font-bold {{ $item->Quantity <= $item->ReorderLevel ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' }}">
                                        {{ $item->Quantity }} <span class="text-xs font-normal text-gray-500">{{ $item->Unit }}</span>
                                    </span>
                                    @if($item->Quantity <= $item->ReorderLevel)
                                        <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800 uppercase tracking-wide w-fit">
                                            Low
                                        </span>
                                    @endif
                                </div>
                            ...
                            <!-- Reorder Point (Hidden Tablet) -->
                            <td class="hidden lg:table-cell px-3 py-4 text-sm text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap">
                                {{ $item->ReorderLevel }} {{ $item->Unit }}
                            </td>

                            <!-- Unit Cost (Hidden Small Mobile) -->
                            <td class="hidden sm:table-cell px-3 py-4 text-sm font-bold text-gray-900 dark:text-white whitespace-nowrap">
