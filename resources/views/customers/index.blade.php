...
@section('content')
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            ...
            <p class="text-gray-500 dark:text-gray-400 font-medium">Manage your client database.</p>
        </div>
        ...
        <div class="flex gap-3 w-full md:w-auto">
            ...
            <form action="{{ route('customers.index') }}" method="GET" class="flex-1 md:w-64">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ $search ?? '' }}" class="block w-full pl-10 pr-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl leading-5 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm shadow-sm" placeholder="Search customers...">
                </div>
            </form>

            <!-- Add New Customer Button -->
            <a href="{{ route('customers.create') }}" 
               class="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 hover:-translate-y-0.5 transition-all duration-200 active:scale-95 active:translate-y-0 focus:outline-none focus:ring-4 focus:ring-blue-500/20 whitespace-nowrap">
            ...
            <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Joined</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse ($customers as $customer)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap"><span class="px-2.5 py-1 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-bold">#{{ $customer->CustomerID }}</span></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-xs">{{ substr($customer->Name, 0, 1) }}</div>
                                    <span class="text-sm font-bold text-gray-900 dark:text-white">{{ $customer->Name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 font-medium">{{ $customer->ContactNumber }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $customer->Email ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ \Carbon\Carbon::parse($customer->DateCreated)->format('M d, Y') }}</td>
